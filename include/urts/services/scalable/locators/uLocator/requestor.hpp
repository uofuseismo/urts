#ifndef URTS_SERVICES_SCALABLE_LOCATORS_ULOCATOR_REQUESTOR_HPP
#define URTS_SERVICES_SCALABLE_LOCATORS_ULOCATOR_REQUESTOR_HPP
#include <memory>
namespace UMPS
{
 namespace Logging
...
namespace URTS::Services::Scalable::Locators::ULocator
{
/// @class Requestor "requestor.hpp" "urts/services/scalable/locators/uLocator/requestor.hpp"
/// @brief A requestor that will submit an earthquake location request.
...
public:
    ...
    /// @brief Performs a blocking location request. 
    /// @param[in] request  The assication request to make to the server via
    ///                     the router. 
    /// @result The response to the location request from the server.
    /// @throws std::runtime_error if \c isInitialized() is false.
    [[nodiscard]] std::unique_ptr<LocationResponse> request(const LocationRequest &request);
    /// @}
