#ifndef URTS_SERVICES_SCALABLE_PACKET_CACHE_REQUESTOR_HPP
#define URTS_SERVICES_SCALABLE_PACKET_CACHE_REQUESTOR_HPP
#include <memory>
...
namespace URTS::Services::Scalable::PacketCache
{
/// @class Requestor "requestor.hpp" "urts/services/scalable/packetCache/requestor.hpp"
...
public:
    ...
    /// @throws std::runtime_error if \c isInitialized() is false.
    [[nodiscard]] std::unique_ptr<DataResponse> request(const DataRequest &request);
    ...
    ///         bulk request.
    /// @throws std::runtime_error if \c isInitialized() is false.
    [[nodiscard]] std::unique_ptr<BulkDataResponse> request(const BulkDataRequest &request);
    /// @}
