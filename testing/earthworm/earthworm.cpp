...
template<class T>
class EarthwormTraceBuf2Test : public testing::Test
{
public:
...
protected:
    EarthwormTraceBuf2Test() :
        traceBuf2(std::make_unique<TraceBuf2<T>> ())
    {
    }
    ~EarthwormTraceBuf2Test() override = default;
    std::unique_ptr<TraceBuf2<T>> traceBuf2;
};


TYPED_TEST_SUITE(EarthwormTraceBuf2Test, MyTypes);

TYPED_TEST(EarthwormTraceBuf2Test, TraceBuf2)
{
