...
template<class T>
class BroadcastsExternalEarthwormTraceBuf2Test : public testing::Test
{
public:
...
protected:
    BroadcastsExternalEarthwormTraceBuf2Test() :
        traceBuf2(std::make_unique<TraceBuf2<T>> ())
    {
    }
    ~BroadcastsExternalEarthwormTraceBuf2Test() override = default;
    std::unique_ptr<TraceBuf2<T>> traceBuf2;
};


TYPED_TEST_SUITE(BroadcastsExternalEarthwormTraceBuf2Test, MyTypes);

TYPED_TEST(BroadcastsExternalEarthwormTraceBuf2Test, TraceBuf2)
{
    ...
    auto traceBackDataPacket = dataPacket.getData();
    EXPECT_EQ(traceBackDataPacket.size(), timeSeries.size());
    for (int i = 0; i < static_cast<int> (traceBackDataPacket.size()); ++i)
    {
        auto res = static_cast<double> (traceBackDataPacket[i] - timeSeries[i]);
        EXPECT_NEAR(res, 0, 1.e-14);
    }
}

TEST(BroadcastsExternalEarthwormTraceBuf2Test, FromEarthworm)
{
