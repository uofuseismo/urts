...
namespace SPicker = URTS::Services::Scalable::Pickers::CNNThreeComponentS;

#define MODULE_NAME "SPickRegressorService"
...
struct ProgramOptions
{
    ...
    void parseInitializationFile(const std::string &iniFile)
    {
        ...
        {
            "/usr/local/share/UUSSMLModels/pickersSRegressor.onnx"
        };
        ...
        weightsFile
            = propertyTree.get<std::string> ("SRegressor.weightsFile",
                                             weightsFile);
        mServiceOptions.setModelWeightsFile(weightsFile);

        SPicker::ServiceOptions::Device device
            = SPicker::ServiceOptions::Device::CPU;
        auto deviceString
            = propertyTree.get<std::string> ("SRegressor.device",
                                             "cpu");
        std::transform(deviceString.begin(), deviceString.end(),
                       deviceString.begin(), ::toupper);
        if (deviceString == std::string {"GPU"})
        {
            device = SPicker::ServiceOptions::Device::GPU;
        }
...
//public:
    SPicker::ServiceOptions mServiceOptions;
    std::string mServiceName{"SPickRegressor"};
...
public:
    ...
    InferenceEngine(const std::string &moduleName,
                    std::unique_ptr<SPicker::Service> &&inferenceService,
...
private:
    ...
    std::unique_ptr<SPicker::Service> mInferenceService{nullptr};
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        auto inferenceService
            = std::make_unique<SPicker::Service> (context, logger);
...
R"""(
The pPickRegressorService regressor allows a module to improve an initial S
