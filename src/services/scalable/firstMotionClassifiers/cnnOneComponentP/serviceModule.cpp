...
namespace UNet = URTS::Services::Scalable::FirstMotionClassifiers::CNNOneComponentP;

#define MODULE_NAME "CNNOneComponentPPickRegressor"
...
struct ProgramOptions
{
    ...
    void parseInitializationFile(const std::string &iniFile)
    {
        ...
        {
            "/usr/local/share/UUSSMLModels/detectorsCNNOneComponentP.onnx"
        };
        ...
        mServiceName
             = propertyTree.get<std::string>
               ("CNNOneComponentP.proxyServiceName", mServiceName);
    
        auto backendAddress
             = propertyTree.get<std::string>
               ("CNNOneComponentP.proxyServiceAddress", "");
        ...
        mServiceOptions.setReceiveHighWaterMark(
            propertyTree.get<int> (
                "CNNOneComponentP.proxyServiceReceiveHighWaterMark",
                mServiceOptions.getReceiveHighWaterMark())
        );
        mServiceOptions.setSendHighWaterMark(
            propertyTree.get<int> (
                "CNNOneComponentP.proxyServiceSendHighWaterMark",
...
inference needs.  Example usage:
    cnn1CFirstMotionService --ini=cnnFirstMotionService.ini --instance=1
Allowed options)""");
    desc.add_options()
