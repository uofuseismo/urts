...
namespace UNet = URTS::Services::Scalable::Detectors::UNetOneComponentP;

#define MODULE_NAME "UNetOneComponentP"
...
struct ProgramOptions
{
    ...
    void parseInitializationFile(const std::string &iniFile)
    {
        ...
        const std::string section{"UNetOneComponentP"};
        weightsFile
            = propertyTree.get<std::string> (section + ".weightsFile",
                                             weightsFile);
        mServiceOptions.setModelWeightsFile(weightsFile);

        UNet::ServiceOptions::Device device = UNet::ServiceOptions::Device::CPU;
        auto deviceString
           = propertyTree.get<std::string> (section + ".device",
                                            "cpu");
...
R"""(
The uNetOneComponentPService allows a detector module to apply the UUSS 
P-wave detector to the single-channel data.  This is a scalable service so it
...
as to satisfy the overall application's inference needs.  Example usage:
    uNetOneComponentPService --ini=uNetOneComponentPService.ini --instance=0
Allowed options)""");
    desc.add_options()
